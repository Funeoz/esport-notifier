...
class Lol(commands.Cog):
    def __init__(self, client):
        self.client = client
        try:
            with open("database/lol/worldRanking.json", "r") as file:
                self.ranking = json.loads(file.read())
        except FileNotFoundError:
            lol_ranking.get_world_ranking()
        with open("database/lol/pastMatches.json", "r") as file:
            self.past_matches = json.loads(file.read())
        with open("database/lol/runningMatches.json", "r") as file:
            self.running_matches = json.loads(file.read())
        with open("database/lol/upcomingMatches.json", "r") as file:
            self.upcoming_matches = json.loads(file.read())
        with open("database/lol/pastEvents.json", "r") as file:
            self.past_events = json.loads(file.read())
        with open("database/lol/upcomingEvents.json", "r") as file:
            self.upcoming_events = json.loads(file.read())
        with open("database/lol/runningEvents.json", "r") as file:
    ...
    async def help(self, ctx):
        embed = discord.Embed(
            title="LOL Commands Help",
            description="Every command below starts with `.en lol`",
        ...
        """embed.add_field(
            name="`ranking`",
            value="Show the GosuGamers Top 30 ranking.",
            inline=False,
        )"""
        embed.add_field(
            name="** **",
    ...
    async def matches(self, ctx, arg):
        ...
        if arg == "past":
            ...
            running_matches_embed = discord.Embed(
                title="Running LOL Matches",
                description=f"📅 {date_re}",
                colour=discord.Colour.dark_gold(),
            )
            running_matches_embed.set_thumbnail(url="https://i.imgur.com/v12TmMh.png")
    ...
    async def matches_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send("⛔ Missing argument. See `.en lol help`.")
    ...
    async def ranking(self, ctx):
        ...
        embed = discord.Embed(
            title="GosuGamers LoL World Ranking",
            url="https://www.gosugamers.net/lol/rankings",
            description="**  **",
            colour=discord.Colour.dark_gold(),
        )
